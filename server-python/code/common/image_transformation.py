...
def apply_image_transformation(frame):
    ...
    #cv2.imshow("Resize image".format(""), frame)
    frame = make_background_black(frame)
    #cv2.imshow("make background black".format(""), frame)
    #cv2.waitKey()
    frame = make_skin_white(frame)
    #cv2.imshow("make skin white".format(""), frame)
    frame = remove_arm(frame)
    #cv2.imshow("Remove arm".format(""), frame)
    frame, contour_dimensions = draw_contours(frame)
    #cv2.imshow("contour dimension".format(""), frame)
    frame = centre_frame(frame, contour_dimensions)
    #cv2.imshow("center frame".format(""), frame)
